...
#[cfg(test)]
mod tests {
    use std::{collections::{BTreeMap, HashMap}, fs::{self, File}, io::Write, path::Path};

    use serde_json::json;

    use crate::race::types::{Coords, Defaults, Flags, PointBuilder, PointTypes, Race, SpecArea};
    ...
    fn test_structs() {
        ...
        file.write_all(format!("{}", json!(race)).as_bytes()).unwrap();
        let mut file = File::create(Path::new("output").join("point_types.json")).unwrap();
        file.write_all(format!("{}", json!(point_types)).as_bytes()).unwrap();
