...
fn download_image(url: &str) {
        ...
        fs::create_dir_all(format!("/Users/lex3man/Projects/FountCore/AS_tauri_version/src-tauri/src/tests/downloaded/{}/", subdir)).unwrap();
        let file_path = format!("/Users/lex3man/Projects/FountCore/AS_tauri_version/src-tauri/src/tests/downloaded/{}/{}", subdir, file_name);
...
fn load_images_test() {
    ...
    let cfg = crate::utils::parser::upload_config(crate::utils::parser::FormatedData::Json(request));
    assert!(cfg.is_some());
    let binding = cfg.unwrap();
    let rb_images = &binding.areas.get("111111").unwrap().roadbook;
    assert_eq!(rb_images.len(), 18);
    fs::create_dir_all("/Users/lex3man/Projects/FountCore/AS_tauri_version/src-tauri/src/tests/downloaded/").unwrap();
